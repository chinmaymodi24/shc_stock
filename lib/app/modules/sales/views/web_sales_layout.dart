                ...
                Expanded(
                  child: Obx(() {
                    ...
                    final filtered = _searchQuery.isEmpty
                        ? all.toList()
                        : all.where((o) =>
                            o.client.toLowerCase().contains(_searchQuery.toLowerCase()) ||
                            o.soNumber.toLowerCase().contains(_searchQuery.toLowerCase()))
                          .toList();
                          ...
                          Row(
                            ...
                            children: [
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text('Manage all your sales orders and track your business performance.',
                              ...
                              _PrimaryBtn(
                                ...
                                onTap: () => Get.to(() => const WebNewSalesLayout(), transition: Transition.fadeIn),
                              ),
                            ],
                          ),
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              SizedBox(
                                ...
                                child: Column(
                                  children: [
                                    _SalesSummaryCard(colors: colors, controller: c),
                                    const SizedBox(height: 14),
                                    _TopClientsCard(colors: colors, controller: c),
                                    const SizedBox(height: 14),
...
class _StatCard extends StatelessWidget {
  ...
  const _StatCard({
    ...
    required this.spark,   this.small = false,
  });
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(label, style: TextStyle(fontSize: 11, color: colors.textSecondary, fontFamily: 'Poppins', fontWeight: FontWeight.w500)),
                const SizedBox(height: 3),
                Text(value, style: TextStyle(fontSize: small ? 16 : 24, fontWeight: FontWeight.w700, color: colors.textPrimary, fontFamily: 'Poppins')),
                ...
                Row(children: [
                  ...
                  Text(trend, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600,
                    color: trendUp ? const Color(0xFF22C55E) : const Color(0xFFEF4444), fontFamily: 'Poppins')),
                  const SizedBox(width: 4),
                  Flexible(child: Text('from last month', style: TextStyle(fontSize: 10, color: colors.textHint, fontFamily: 'Poppins'), overflow: TextOverflow.ellipsis)),
                ]),
              ],
            ),
          ),
...
class _SparklinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final lp = Paint()..color = color..strokeWidth = 1.8..style = PaintingStyle.stroke..strokeCap = StrokeCap.round;
...
class _TableCard extends StatelessWidget {
  ...
  final int startIdx, total, startDisplay, endDisplay, currentPage, totalPages, rowsPerPage;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        children: [
          // Toolbar
          _Toolbar(colors: colors, searchCtrl: searchCtrl, onSearch: onSearch),
          ...
          if (pageItems.isEmpty)
            ...
            ...pageItems.asMap().entries.map((e) => _SalesRow(
              order: e.value,
          ...
          _Footer(
            colors: colors,
            total: total, startDisplay: startDisplay, endDisplay: endDisplay,
            currentPage: currentPage, totalPages: totalPages, rowsPerPage: rowsPerPage,
            onPageChanged: onPageChanged, onRowsChanged: onRowsChanged,
          ),
        ],
      ),
    );
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const Spacer(),
          _OutlineBtn(label: 'Filters', icon: Icons.tune_rounded, colors: colors),
          const SizedBox(width: 8),
          // Date range
          Container(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(Icons.date_range_rounded, size: 15, color: colors.textSecondary),
              const SizedBox(width: 6),
              Text('01 May 2024 - 31 May 2024', style: TextStyle(fontSize: 12.5, color: colors.textSecondary, fontFamily: 'Poppins')),
              const SizedBox(width: 6),
              Icon(Icons.keyboard_arrow_down_rounded, size: 16, color: colors.textSecondary),
            ]),
          ),
          const SizedBox(width: 8),
          _OutlineBtn(label: 'Export', icon: Icons.upload_outlined, colors: colors),
          const SizedBox(width: 8),
          Container(
            width: 34, height: 34,
            decoration: BoxDecoration(color: colors.inputFill, border: Border.all(color: colors.border), borderRadius: BorderRadius.circular(7)),
...
class _OutlineBtn extends StatelessWidget {
  ...
  final IconData icon;
  final AppThemeColors colors;
  const _OutlineBtn({required this.label, required this.icon, required this.colors});
  ...
  Widget build(BuildContext context) {
    return OutlinedButton.icon(
      ...
      label: Text(label, style: TextStyle(fontSize: 13, fontFamily: 'Poppins', color: colors.textSecondary)),
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        children: [
          // # column
          SizedBox(width: _kIdxWidth, child: Text('#', style: _s)),
          SizedBox(width: _kColGap),
          // SO Number
          Expanded(flex: _kSoFlex, child: Text('SO Number', style: _s)),
          // Client
          Expanded(flex: _kClientFlex, child: Text('Client', style: _s)),
          // Date
          Expanded(flex: _kDateFlex, child: Text('Date', style: _s)),
          // Items (center)
          Expanded(flex: _kItemsFlex, child: Center(child: Text('Items', style: _s))),
          // Amount
          Expanded(flex: _kAmtFlex, child: Text('Amount', style: _s)),
          // Status (center)
          Expanded(flex: _kStatusFlex, child: Center(child: Text('Status', style: _s))),
          // Payment Status (center)
          Expanded(flex: _kPayFlex, child: Center(child: Text('Payment Status', style: _s.copyWith(fontSize: 11.5)))),
          // Actions (center)
          ...
          child: Row(
            ...
            children: [
              ...
              SizedBox(
                width: _kIdxWidth,
                child: Container(
                  width: _kIdxWidth, height: _kIdxWidth,
                  decoration: BoxDecoration(
                  ...
                  child: Center(child: Text(
                    '${widget.displayIndex}',
                    style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.primaryOrange, fontFamily: 'Poppins'),
                  )),
                ),
              ),
              SizedBox(width: _kColGap),
              ...
              // SO Number
              Expanded(flex: _kSoFlex, child: Text(o.soNumber,
                style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Color(0xFF4A3AFF), fontFamily: 'Poppins'),
                overflow: TextOverflow.ellipsis,
              )),

              // Client
              Expanded(flex: _kClientFlex, child: Row(children: [
                Container(
                  width: 32, height: 32,
                  decoration: BoxDecoration(color: o.clientColor.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(8)),
                  child: Center(child: Text(o.clientBadge,
                    style: TextStyle(fontSize: o.clientBadge.length > 2 ? 9.5 : 11, fontWeight: FontWeight.w700, color: o.clientColor, fontFamily: 'Poppins'),
                  )),
                ),
                const SizedBox(width: 8),
                Expanded(child: Text(o.client,
                  style: TextStyle(fontSize: 13.5, fontWeight: FontWeight.w500, color: c.textPrimary, fontFamily: 'Poppins'),
                  overflow: TextOverflow.ellipsis,
                )),
              ])),

              // Date
              Expanded(flex: _kDateFlex, child: Text(_fmtDate(o.date),
                style: TextStyle(fontSize: 13, color: c.textSecondary, fontFamily: 'Poppins'),
              )),

              // Items
              Expanded(flex: _kItemsFlex, child: Center(child: Text('${o.itemCount}',
                style: TextStyle(fontSize: 13.5, fontWeight: FontWeight.w600, color: c.textPrimary, fontFamily: 'Poppins'),
              ))),

              // Amount
              Expanded(flex: _kAmtFlex, child: Text(_fmtAmount(o.amount),
                style: TextStyle(fontSize: 13.5, fontWeight: FontWeight.w600, color: c.textPrimary, fontFamily: 'Poppins'),
              )),

              // Status badge
              Expanded(flex: _kStatusFlex, child: Center(child: _Badge(label: o.status.label, color: o.status.color))),

              // Payment Status badge
              Expanded(flex: _kPayFlex, child: Center(child: _Badge(label: o.paymentStatus.label, color: o.paymentStatus.color))),

              // Actions
              Expanded(flex: _kActFlex, child: Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  _ActionBtn(icon: Icons.remove_red_eye_outlined, color: const Color(0xFF4A3AFF), bg: const Color(0xFF4A3AFF), onTap: () {}),
                  const SizedBox(width: 6),
                  _ActionBtn(icon: Icons.more_vert_rounded, color: c.textSecondary, bg: c.iconBgPurple, onTap: () {}),
                ],
              )),
            ],
          ),
...
class _Badge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(label, style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w600, color: color, fontFamily: 'Poppins')),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return Tooltip(
      ...
      child: InkWell(
        ...
        child: Container(
          width: 30, height: 30,
          decoration: BoxDecoration(color: bg.withValues(alpha: icon == Icons.more_vert_rounded ? 1.0 : 0.10), borderRadius: BorderRadius.circular(7)),
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        Icon(Icons.point_of_sale_outlined, size: 40, color: colors.textHint),
        const SizedBox(height: 10),
        Text('No sales orders found', style: TextStyle(fontSize: 14, color: colors.textHint, fontFamily: 'Poppins')),
      ])),
    );
  }
}
...
class _Footer extends StatelessWidget {
  ...
  const _Footer({
    ...
    required this.onPageChanged, required this.onRowsChanged,
  });
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        children: [
          Text('Showing $startDisplay to $endDisplay of $total sales orders',
              ...
              child: GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  child: Center(child: Text('$page', style: TextStyle(
                    ...
                    color: isActive ? Colors.white : colors.textPrimary, fontFamily: 'Poppins',
                  ))),
                ),
              ),
            );
          ...
          // ellipsis
          if (totalPages > 3) ...[
            Text('...', style: TextStyle(fontSize: 14, color: colors.textSecondary, fontFamily: 'Poppins')),
            const SizedBox(width: 4),
            GestureDetector(
              ...
              child: Container(
                width: 32, height: 32,
                decoration: BoxDecoration(
                  color: currentPage == totalPages ? AppColors.primaryOrange : colors.surface,
                  borderRadius: BorderRadius.circular(7),
                  border: Border.all(color: currentPage == totalPages ? AppColors.primaryOrange : colors.border),
                ),
                child: Center(child: Text('$totalPages', style: TextStyle(
                  fontSize: 13, fontWeight: FontWeight.w700,
                  color: currentPage == totalPages ? Colors.white : colors.textPrimary, fontFamily: 'Poppins',
                ))),
              ),
            ),
            const SizedBox(width: 4),
...
class _SalesSummaryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _PanelCard(
      ...
      child: Column(
        children: [
          _SummaryRow(label: 'Total Sales Orders',  value: '${controller.totalOrders}', colors: colors),
          const SizedBox(height: 10),
          _SummaryRow(label: 'Total Sales Amount',  value: '₹ 24,85,600', colors: colors),
          const SizedBox(height: 10),
          _SummaryRow(label: 'Total Received',      value: '₹ 21,60,200', colors: colors),
          const SizedBox(height: 10),
          _SummaryRow(label: 'Total Due',           value: '₹ 3,25,400',  colors: colors),
          const SizedBox(height: 10),
          _SummaryRow(label: 'Average Order Value', value: '₹ 15,932',    colors: colors),
        ],
      ),
    );
  }
...
class _SummaryRow extends StatelessWidget {
  ...
  final AppThemeColors colors;
  const _SummaryRow({required this.label, required this.value, required this.colors});
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        Flexible(child: Text(label, style: TextStyle(fontSize: 12.5, color: colors.textSecondary, fontFamily: 'Poppins'), overflow: TextOverflow.ellipsis)),
        const SizedBox(width: 8),
        Text(value, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: colors.textPrimary, fontFamily: 'Poppins')),
      ],
    );
  }
}
    ...
    return _PanelCard(
      ...
      action: GestureDetector(
        ...
        child: const Text('View All',
          style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w600, color: AppColors.primaryOrange, fontFamily: 'Poppins')),
      ),
      child: Column(
        children: controller.topClients.map((tc) => Padding(
          ...
          child: Row(
            children: [
              ...
              Container(
                width: 38, height: 38,
                decoration: BoxDecoration(color: tc.color.withValues(alpha: 0.15), shape: BoxShape.circle),
                child: Center(child: Text(tc.badge,
                  style: TextStyle(fontSize: tc.badge.length > 2 ? 9 : 11, fontWeight: FontWeight.w800, color: tc.color, fontFamily: 'Poppins'),
                )),
              ),
              const SizedBox(width: 10),
              // Name + orders
              Expanded(child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(tc.name, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: colors.textPrimary, fontFamily: 'Poppins'), overflow: TextOverflow.ellipsis),
                  Text('${tc.orderCount} Orders', style: TextStyle(fontSize: 11, color: colors.textSecondary, fontFamily: 'Poppins')),
                ],
              )),
              const SizedBox(width: 8),
              // Total amount
              Text(_fmtAmt(tc.totalAmount), style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, color: colors.textPrimary, fontFamily: 'Poppins')),
            ],
          ),
        )).toList(),
      ),
...
class _PanelCard extends StatelessWidget {
  ...
  final Widget child;
  final Widget? action;
  const _PanelCard({required this.colors, required this.title, required this.child, this.action});
...
class _PrimaryBtn extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ElevatedButton.icon(
      ...
      label: Text(label, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Colors.white, fontFamily: 'Poppins')),
      style: ElevatedButton.styleFrom(
        backgroundColor: AppColors.primaryOrange, elevation: 0,
