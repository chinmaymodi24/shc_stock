...
class AppDrawer extends StatelessWidget {
  ...
  static const _items = [
    _DrawerItem(icon: Icons.dashboard_rounded,        label: 'Dashboard',  route: AppRoutes.dashboard),
    _DrawerItem(icon: Icons.inventory_2_outlined,     label: 'Products',   route: AppRoutes.products),
    _DrawerItem(icon: Icons.category_outlined,        label: 'Categories', route: AppRoutes.categories),
    _DrawerItem(icon: Icons.warehouse_outlined,       label: 'Stock',      route: AppRoutes.stock),
    _DrawerItem(icon: Icons.shopping_bag_outlined,    label: 'Purchase',   route: AppRoutes.purchase),
    _DrawerItem(icon: Icons.point_of_sale_outlined,   label: 'Sales',      route: AppRoutes.sales),
    _DrawerItem(icon: Icons.people_outline_rounded,   label: 'Clients',    route: AppRoutes.clients),
    _DrawerItem(icon: Icons.bar_chart_rounded,        label: 'Reports',    route: AppRoutes.reports),
    _DrawerItem(icon: Icons.settings_outlined,        label: 'Settings',   route: AppRoutes.settings),
  ];
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  Image.asset('assets/logo.png', width: 100, fit: BoxFit.fitWidth),
                  const SizedBox(height: 16),
                  const CircleAvatar(
                    radius: 24,
                    backgroundColor: Color(0x33F47B20),
                    child: Icon(Icons.person_rounded, color: AppColors.primaryOrange, size: 26),
                  ),
                  const SizedBox(height: 10),
                  const Text('Admin', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Colors.white, fontFamily: 'Poppins')),
                  const Text('admin@secureheatcare.com', style: TextStyle(fontSize: 12, color: Color(0xFFB0AECF), fontFamily: 'Poppins')),
                ],
              ),
            ),
...
class _DrawerTile extends StatelessWidget {
  ...
  const _DrawerTile({required this.item, required this.isActive, required this.isEnabled, required this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Material(
        color: isActive ? AppColors.primaryOrange.withValues(alpha: 0.1) : Colors.transparent,
