...
String displayReportValue(String label, String raw) {
  const moneyLabels = {
    'Total Sales',
    'Receivable',
    'Average Order Value',
...
final reportDateFmt = DateFormat('dd MMM yyyy');

/// Net Movement card — shared between web and mobile.
Widget reportNetMovementCard(AppThemeColors colors, ReportsController c) {
  final value = double.tryParse(c.netMovement.value) ?? 0;
...
// ─────────────────────────────────────────────────────────────────────────────
class WebReportsLayout extends GetView<ReportsController> {
  const WebReportsLayout({super.key});
...
class ReportRankCard extends StatelessWidget {
  final String title;
  final String subtitle;
  final List<ReportRank> rows;
  final bool money;

  const ReportRankCard({
    super.key,
    required this.title,
